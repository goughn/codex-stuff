...
// SIMPLE test extraction - just look for basic patterns in plain text
function extractSimpleTests(plainText, debugInfo) {
    const tests = [];
    debugInfo.push("📝 Simple text extraction starting...");
    
    try {
        let testId = 1;
        
        // Pattern 1: function(args) → result  
        const pattern1 = /(\w+)\s*\(\s*([^)]*)\s*\)\s*[→->]\s*(\w+|\d+)/g;
        let match;
        
        while ((match = pattern1.exec(plainText)) !== null) {
            const [full, funcName, args, result] = match;
            tests.push({
                id: testId++,
                description: `Test: ${funcName}(${args}) → ${result}`,
                test: `result = ${funcName}(${args})\nassert str(result) == "${result}", f"Expected '${result}', got '{result}'"`
            });
            debugInfo.push(`📝 Found: ${funcName}(${args}) → ${result}`);
        }
        
        // Pattern 2: function(args) returns result
        const pattern2 = /(\w+)\s*\(\s*([^)]*)\s*\)\s+returns?\s+(\w+|\d+)/gi;
        
        while ((match = pattern2.exec(plainText)) !== null) {
            const [full, funcName, args, result] = match;
            // Skip if already found
            if (!tests.some(t => t.test.includes(`${funcName}(${args})`))) {
                tests.push({
                    id: testId++,
                    description: `Test: ${funcName}(${args}) returns ${result}`,
                    test: `result = ${funcName}(${args})\nassert str(result) == "${result}", f"Expected '${result}', got '{result}'"`
                });
                debugInfo.push(`📝 Found: ${funcName}(${args}) returns ${result}`);
            }
        }
        
        // Pattern 3: function(args) should return result
        const pattern3 = /(\w+)\s*\(\s*([^)]*)\s*\)\s+should\s+return\s+(\w+|\d+)/gi;
        
        while ((match = pattern3.exec(plainText)) !== null) {
            const [full, funcName, args, result] = match;
            if (!tests.some(t => t.test.includes(`${funcName}(${args})`))) {
                tests.push({
                    id: testId++,
                    description: `Test: ${funcName}(${args}) should return ${result}`,
                    test: `result = ${funcName}(${args})\nassert str(result) == "${result}", f"Expected '${result}', got '{result}'"`
                });
                debugInfo.push(`📝 Found: ${funcName}(${args}) should return ${result}`);
            }
        }
        
        // Pattern 4: Example: function(args) = result
        const pattern4 = /example[:\s]*(\w+)\s*\(\s*([^)]*)\s*\)\s*[=:]\s*(\w+|\d+)/gi;
        
        while ((match = pattern4.exec(plainText)) !== null) {
            const [full, funcName, args, result] = match;
            if (!tests.some(t => t.test.includes(`${funcName}(${args})`))) {
                tests.push({
                    id: testId++,
                    description: `Example: ${funcName}(${args}) = ${result}`,
                    test: `result = ${funcName}(${args})\nassert str(result) == "${result}", f"Expected '${result}', got '{result}'"`
                });
                debugInfo.push(`📝 Found: ${funcName}(${args}) = ${result}`);
            }
        ...
        console.log(`📝 Simple extraction found ${tests.length} tests`);
        
    } catch (e) {
        debugInfo.push(`❌ Error in simple extraction: ${e.message}`);
        console.log("❌ Simple extraction error:", e);
    }
...
function getTestsFromEvaluationData(itemInstance) {
    ...
    // Method 3: EXTRACT FROM data-tests ATTRIBUTE in source HTML
    if (instanceObj && instanceObj.itemcontent) {
        debugInfo.push("Extracting from data-tests attribute...");
        try {
            console.log("🔍 Looking for data-tests in itemcontent...");
            
            // Look for data-tests attribute in the HTML
            const dataTestsMatch = instanceObj.itemcontent.match(/data-tests\s*=\s*'([^']+)'/);
            
            if (dataTestsMatch) {
                console.log("✅ Found data-tests attribute!");
                const testDataJson = dataTestsMatch[1];
                console.log("📋 Raw test data:", testDataJson.substring(0, 200) + "...");
                
                try {
                    tests = JSON.parse(testDataJson);
                    console.log("✅ Successfully parsed data-tests JSON:", tests.length, "tests");
                    statusMessage = `✅ Tests loaded from data-tests attribute (${tests.length} tests)`;
                    return tests;
                } catch (parseError) {
                    console.log("❌ Error parsing data-tests JSON:", parseError);
                    debugInfo.push(`Error parsing data-tests: ${parseError.message}`);
                }
            } else {
                console.log("❌ No data-tests attribute found");
                debugInfo.push("No data-tests attribute found in itemcontent");
            }
...
function saveAnswer_106(button) {
    console.log("🚀 saveAnswer_106 STARTED");
    console.log("📥 Input button:", button);
    
    var respObject = {};
    itemInstance = button.closest(".itemInstance");
    console.log("🔍 Found itemInstance:", {
        id: itemInstance?.id,
        exists: !!itemInstance
    });
    
    // Safely get the answer element
    const answerElement = itemInstance.querySelector(".answer");
    if (!answerElement) {
        console.error("❌ saveAnswer_106 FAILED: No answer element found");
        console.log("🔍 DOM structure:", itemInstance.innerHTML.substring(0, 200));
        const errorResponse = JSON.stringify({ error: "No answer element found" });
        console.log("📤 saveAnswer_106 RETURNING ERROR:", errorResponse);
        return errorResponse;
    }
    
    pyCode = answerElement.value;
    respObject.code = pyCode;
    console.log("📝 User code captured:", {
        codeLength: pyCode?.length || 0,
        codePreview: pyCode?.substring(0, 100) + "..."
    });

    // Get the output div and its content
    const outputDivId = "o" + itemInstance.id;
    let outputDiv = document.getElementById(outputDivId);
    console.log("🔍 Looking for output div:", {
        expectedId: outputDivId,
        found: !!outputDiv,
        hasTextContent: outputDiv ? (outputDiv.textContent !== null) : false
    });
    
    if (outputDiv && outputDiv.textContent !== null) {
        ...
        console.log("⚠️ Output div not found or has no textContent:", {
            outputDiv: !!outputDiv,
            itemInstanceId: itemInstance.id,
            textContentExists: outputDiv ? (outputDiv.textContent !== null) : "div not found"
        });
        respObject.output = "";
    }
    ...
    console.log("🧪 Checking for test results:", {
        hasTestResults: !!itemInstance._testResults,
        testResults: itemInstance._testResults
    });
    ...
    if (itemInstance._testResults) {
        ...
        console.log("✅ Test results included:", {
            ...
            totalTests,
            maxPossibleMarks,
            percentage: respObject.percentage
        });
        
        // Display marks immediately when Send is clicked
        console.log("🎯 Displaying marks for Send action");
        ...
        console.log("⚠️ No test results found - tests not run yet");
    }
    ...
    const finalResponse = JSON.stringify(respObject);
    console.log("📦 Final response object:", respObject);
    console.log("📤 JSON response to server:", finalResponse);
    console.log("✅ saveAnswer_106 COMPLETED SUCCESSFULLY");
    console.log("=====================================");
...
async function runPythonTests(button, tests) {
    console.log("🧪 runPythonTests STARTED");
    console.log("📥 Input parameters:", {
        button: !!button,
        testsLength: tests?.length || 0,
        testsPreview: tests?.slice(0, 2)
    });
    ...
    if (!tests || tests.length === 0) {
        console.error("❌ runPythonTests FAILED: No tests available");
        ...
        console.log("📤 runPythonTests ENDED: No tests to run");
        return;
    }
    
    console.log("✅ Tests available, proceeding with execution");
    ...
    try {
        ...
        console.log("📊 Test scoring setup:", {
        ...
        console.log("🧪 Running first test:", {
            testIndex: 0,
            mark: firstTestMark,
            testContent: tests[0].test?.substring(0, 100)
        });
        ...
        try {
            ...
            outputDiv.textContent += `\n${error}`;
            console.log("❌ First test FAILED:", error.toString());
        }
        ...
        if (tests.length > 1) {
            ...
            console.log(`🔄 Running ${remainingTests.length} additional tests silently...`);
            
            for (let i = 0; i < remainingTests.length; i++) {
                const test = remainingTests[i];
                const testMark = test.mark || 1;
                const testIndex = i + 1; // +1 because first test was index 0
                
                console.log(`🧪 Running test ${testIndex}:`, {
                    mark: testMark,
                    testContent: test.test?.substring(0, 50) + "..."
                });
                ...
                try {
                    ...
                    totalMarks += testMark;
                    console.log(`✅ Test ${testIndex} PASSED (+${testMark} marks)`);
                } catch (error) {
                    // Do not update outputDiv, just ignore or log
                    console.log(`❌ Test ${testIndex} FAILED:`, error.toString());
                }
                // Don't show marks during testing - only store results
            }
            console.log("🔄 All additional tests completed");
        } else {
            console.log("ℹ️ Only one test to run (no additional tests)");
        }
    } catch (error) {
    ...
    itemInstance._testResults = testResults;
    console.log("💾 Test results stored:", testResults);
    console.log("📊 Final score:", {
        earned: totalMarks,
        possible: maxPossibleMarks,
        percentage: Math.round((totalMarks / maxPossibleMarks) * 100) + "%"
    });
    
    // Show completion message without revealing marks
    outputDiv.textContent += `\n\n✅ Tests completed successfully!\n📝 Click 'Send' to submit your answer and see your score.`;
    console.log("✅ runPythonTests COMPLETED SUCCESSFULLY");
    console.log("=====================================");
}
